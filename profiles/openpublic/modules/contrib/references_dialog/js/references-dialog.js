          ...
          $('.' + key + ' a.references-dialog-activate', context).click(function (e) {
            ...
            Drupal.ReferencesDialog.entityIdReceived = function (entity_type, entity_id, label) {
              ...
              // that invoked this.
                ...
                // Grab nid and node title
                var matches = value.match(/^(?:\s*|(.*) )?\[\s*nid\s*:\s*(\d+)\s*\]$/);
                var nid = matches[2];
                var ntitle = matches[1]
                // Check for deleted node
                if (typeof ntitle == 'undefined') {
                  value = '';
                }
                else {
                  // Swap Create link for Edit link
                  if ($('.' + key + ' > .references-dialog-links .add-dialog').length) {
                    $('.' + key + ' > .references-dialog-links .add-dialog').text('Edit');
                    $('.' + key + ' > .references-dialog-links .add-dialog').attr('href', '/node/' + nid + '/edit');
                    $('.' + key + ' > .references-dialog-links .add-dialog').removeClass('add-dialog').addClass('edit-dialog');
                  }
                }              
                var key_el = $('#' + key);
                key_el.val(value);
